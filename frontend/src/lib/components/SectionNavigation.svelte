...
<div class="relative shrink-0">
  <button
    bind:this={button}
    type="button"
    class="inline-flex h-10 max-w-[9.5rem] items-center gap-2 rounded-full border border-[var(--border-soft)] bg-[var(--surface-strong)] px-3 text-[12px] font-bold uppercase tracking-[0.1em] text-[var(--foreground)] shadow-sm transition-colors hover:border-[var(--border)] lg:hidden"
    aria-expanded={open}
    aria-haspopup="menu"
  ...
  {#if open}
    <div
      ...
      role="menu"
      aria-label="Sections"
      class="absolute right-0 top-full z-[70] mt-2 flex w-48 flex-col gap-1 rounded-[var(--radius-md)] border border-[var(--border-soft)] bg-[var(--surface-strong)] p-1.5 shadow-xl fade-in lg:hidden"
    >
