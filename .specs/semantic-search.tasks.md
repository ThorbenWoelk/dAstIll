...
- Search embedding calls now fan out into bounded `/api/embed` sub-batches with a longer per-request timeout so the worker does not fail an entire indexing round on a single oversized local Ollama request.
...
- Semantic search now defaults on in local debug runs and off in release / production builds. `SEARCH_SEMANTIC_ENABLED` remains the explicit override in either direction, so local developers can still disable it and prod can still opt in intentionally.
