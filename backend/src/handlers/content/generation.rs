...
async fn ensure_summary_internal(
    ...
    let video = require_video(state, video_id).await?;
    {
        if !force_regenerate {
                ...
                .map_err(map_db_err)?
            {
                if allow_cached_auto_regen {
                    let auto_regen_attempts =
                        db::get_summary_auto_regen_attempts(&state.db, video_id)
                            .await
                            .map_err(map_db_err)?;
                    if should_auto_regenerate_summary(
                        ...
                        let _ = db::update_video_summary_status(
                            &state.db,
                            video_id,
                            ContentStatus::Ready,
                        )
            ...
            // Explicit user regenerate: keep the existing summary document until a
            // replacement is successfully generated and upserted below.
            db::reset_summary_auto_regen_attempts(&state.db, video_id)
                .await
                .map_err(map_db_err)?;
            tracing::info!(video_id = %video_id, "summary force-regeneration requested");
        }
...
pub(crate) async fn ensure_summary(
    ...
    ensure_summary_internal(state, video_id, false, false).await
}
...
pub(crate) async fn ensure_summary_for_queue(
    ...
    ensure_summary_internal(state, video_id, true, false).await
}

pub(crate) async fn ensure_summary_force_regenerate(
    state: &AppState,
    video_id: &str,
) -> Result<Summary, (StatusCode, String)> {
    ensure_summary_internal(state, video_id, false, true).await
}
