use super::{
    ...
    DbInspectToolInput, HighlightLookupQuery, HighlightLookupToolInput, RecentLibraryActivityScope,
    RecentLibraryActivityToolInput, SearchLibraryToolInput, breakdown_scope_by_channel,
    build_db_inspect_query, build_highlight_lookup_query, build_recent_library_activity_query,
    build_search_library_query, count_db_inspect_scope, describe_db_inspect_query,
    describe_highlight_lookup_query, format_db_inspect_list_output,
...
fn db_inspect_folds_other_video_scope_into_virtual_others_channel() {
    let scope = DbInspectScope {
        ...
        transcripts: Vec::new(),
        visible_channel_names: HashMap::from([("channel-a".to_string(), "Visible".to_string())]),
