use super::{
    ActiveChatHandle, CHAT_CLASSIFY_TIMEOUT, CHAT_RECENT_ACTIVITY_SOURCE_LIMIT, ChatQueryIntent,
    ChatQueryPlanResponse, ChatRetrievalPlan, ChatService, ChatToolLoopResponse, ChatTurnState,
    PlannedChatToolCall, RetrievedChatSource, ToolLoopAction, is_direct_video_lookup_request,
...
fn tool_loop_builds_db_inspect_query_when_requested() {
    ...
    let ToolLoopAction::ToolCall(PlannedChatToolCall::DbInspect(query)) = execution.action else {
...
fn tool_loop_accepts_direct_highlight_lookup_action() {
    ...
    let ToolLoopAction::ToolCall(PlannedChatToolCall::HighlightLookup(query)) = execution.action
    else {
...
fn direct_highlight_tool_call_uses_scoped_video_title() {
    ...
    let Some(PlannedChatToolCall::HighlightLookup(query)) = maybe_direct_highlight_lookup_tool_call(
