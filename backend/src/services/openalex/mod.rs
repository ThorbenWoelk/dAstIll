...
/// Stable saved-search id for the shared catalog.
///
/// Slugifying `query_text` alone collapses distinct queries (`C++` vs `C#`,
/// `machine_learning` vs `machine-learning`) and also ignores filters/sort/scope.
/// Later subscribe/sync then overwrites the shared channel + structured query.
/// Always append a fingerprint of the exact query identity.
pub fn saved_search_query_id(query: &OpenAlexSavedSearchQuery) -> String {
    format!(
        "{}:{}",
        slugify_query(&query.query_text),
        saved_search_fingerprint(query)
    )
}
