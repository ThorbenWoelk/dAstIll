...
The repo also manages AWS resources, so CI needs short-lived AWS credentials for Terraform
in addition to the existing GitHub -> GCP federation.
...
CI authenticates to AWS through GitHub OIDC and a dedicated Terraform role instead of
reusing the Cloud Run backend runtime role.
...
- gives Terraform in CI a distinct AWS trust path without long-lived AWS keys
...
- first creation of the AWS GitHub OIDC provider and CI role still requires one authenticated bootstrap apply
...
Keep GitHub CI AWS access and Cloud Run runtime AWS access as separate IAM roles. Do not
collapse them back into one trust boundary.
