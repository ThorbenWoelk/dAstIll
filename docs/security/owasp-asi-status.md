...
| ASI02 - Tool Misuse & Exploitation         | Partially addressed                       | Read-only tool set, schema-bounded inputs, bounded tool loops, scoped retrieval tools, authenticated-only `db_inspect`                     | Add distributed rate and budget enforcement, tool-call audit trails, and stronger anomaly detection for repeated tool abuse                                              |
