...
class TestMonitoringService(unittest.TestCase):
    ...
    def test_setup_channels_success(self):
        """Test successful channel setup."""
        # Mock the RSS monitor instance
        mock_monitor = Mock()
        mock_monitor.resolve_channel_id.return_value = "UC_resolved123"
        self.service.rss_monitor = mock_monitor
        
        # Add a channel without channel ID
        self.service.config_manager.add_channel("Test Channel", "@testchannel")
        self.service.config_manager.set_global_monitoring(True)
        
        # Setup channels
        result = self.service.setup_channels()
        
        self.assertTrue(result)
        mock_monitor.resolve_channel_id.assert_called_once_with("@testchannel")
        
        # Verify channel ID was updated
        channel = self.service.config_manager.get_channel("@testchannel")
        self.assertEqual(channel.channel_id, "UC_resolved123")
    
    def test_setup_channels_resolution_failure(self):
        """Test channel setup when resolution fails."""
        # Mock RSS monitor to fail resolution
        mock_monitor = Mock()
        mock_monitor.resolve_channel_id.return_value = None
        self.service.rss_monitor = mock_monitor
        
        # Add a channel without channel ID
        self.service.config_manager.add_channel("Test Channel", "@testchannel")
        
        # Setup channels should fail
        result = self.service.setup_channels()
    ...
    def test_test_configuration(self):
        ...
        mock_monitor.get_latest_videos.return_value = [
            VideoInfo("video123", "Test Video", "2024-01-01", "Test Channel", "UC_test", "")
        ]
        self.service.rss_monitor = mock_monitor
        
        # Add a channel with channel ID
        self.service.config_manager.add_channel("Test Channel", "@testchannel")
        self.service.config_manager.update_channel_id("@testchannel", "UC_test123")
    ...
    def test_start_monitoring_success(self, mock_rss_monitor_class):
        ...
        # Mock RSS monitor
        mock_monitor = Mock()
        mock_monitor.resolve_channel_id.return_value = "UC_test123"
        mock_monitor.test_rss_feed.return_value = True
    ...
    def test_check_channel_new_video(self):
        ...
        # Mock RSS monitor
        mock_monitor = Mock()
        test_video = VideoInfo("new123", "New Video", "2024-01-01", "Test Channel", "UC_test", "")
    ...
    def test_check_channel_no_new_videos(self, mock_rss_monitor_class):
        ...
        # Mock RSS monitor
        mock_monitor = Mock()
        existing_video = VideoInfo("existing123", "Existing Video", "2024-01-01", "Test Channel", "UC_test", "")
    ...
    def test_process_new_video_auto_download(self):
        ...
        video = VideoInfo("test123", "Test Video", "2024-01-01", "Test Channel", "UC_test", "")
        channel = ChannelConfig(
            name="Test Channel",
            handle="@testchannel",
